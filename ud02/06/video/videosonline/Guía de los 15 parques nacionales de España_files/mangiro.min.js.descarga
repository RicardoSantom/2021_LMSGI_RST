...
(function (root, factory) {
    ...
    var Utils = function () {
        ...
        return {
                        ...
                        if (null != data) {
                            try {
                                ...
                                $.each(data.entry_data.ProfilePage[0].graphql.user.edge_owner_to_timeline_media.edges, function (key, val) {
                                    if (iIndex++ < imagesCount && val.node.edge_media_to_caption.edges.length > 0) {
                                        items.push("<li>");
            ...
            loadTagsTagType: function (capital) {
                $.ajax({
                    type: 'POST',
                    url: '/etiquetaTipo.aspx/GetTags',
    ...
    var Social = function () {
        ...
        function Social(o) {
            ...
            this.shareServices.push({
                ...
                selector: "a.wha",
                action: function (d, that) {
                    var documentUrl = "whatsapp://send?text=";
                    if (d.text != undefined && d.text != "undefined" && d.text != "") documentUrl += d.text + " ";
                    documentUrl += d.url;
    ...
    var Misc = function () {
        ...
        $.extend(Misc.prototype, {
            ...
            initNewsletterSubscription: function (options) {
                ...
                this.newsletterConfig.selectError = "Debe seleccionar la Newsletter a la que suscribirse.";
                        ...
                        if ($(that.chkNewsletterList).length > 0) {
                            ...
                            for (var i = 0; i < newsletters.length; i++) {
                                ...
                                if (newsletter.checked == true) {
                                    that.chkSelector = (":checked");
                                }
                            }
                            if (!$(that.chkSelector).is(":checked")) {
                                showTooltip(that.txtSelector, that.selectError);
                                _datalayer.trackNewsletter(that.title, "error-condiciones");
                                return false;
                            }
                        } else if (!$(that.chkSelector).is(":checked")) {
            ...
            initNewsletterPopUpSubscription: function (options) {
                ...
                this.newsletterPopUpConfig.chkSelectorOthers = ".chkPopUpExtra";
                this.newsletterPopUpConfig.chkNewsletterList = ".chkPopUpNewsletter";
                this.newsletterPopUpConfig.selectNewsletterList = ".slctPopUpNewsletters";
                ...
                require(['jqueryui-tooltip'], function () {
                    ...
                    var forcePostSuscribePopUp = function (evt) {
                        ...
                        if ($(that.chkSelectorOthers).length > 0) {
                            ...
                            for (var i = 0; i < checks.length; i++) {
                                var check = checks[i];
                                if (check.checked == true) {
                                    extraCheck += check.value + ";";
                                }
                            }
                        }

                        var inputListNewsletters = "";
                        //Comprobamos si hay newsletter seleccionado
                        if ($(that.chkNewsletterList).length > 0) {
                            //Lista Newsletters                            
                            var newslettersChecks = $(that.chkNewsletterList);
                            for (var i = 0; i < newslettersChecks.length; i++) {
                                var newsletterCheck = newslettersChecks[i];
                                if (newsletterCheck.checked == true) {
                                    inputListNewsletters += newsletterCheck.value + ";";
                                }
                            }
                        }
                        else if ($(that.selectNewsletterList).length > 0) {
                            //Lista Newsletters                            
                            var nLetter = $(that.selectNewsletterList)[0];
                            inputListNewsletters = nLetter.value;
                        }
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                'a': 'subscribe',
                                'c': crm,
                                'listNewsletters': inputListNewsletters,
                    ...
                    require(['trackPopup'], function (trackPopup) {

                        // Click on banner
                        if (trackPopup.isArticle()) {
                            var element = $('#popUp-content a')[0];

                            if (typeof element !== 'undefined') {
                                element.addEventListener("click", () => { trackPopup.track('click'); });
                            }
                        }

                        var initPopUp = function () {
                            $('.popUp-newsletter').removeClass("ocultar");
                            trackPopup.track('show');
                        ...
                        var showPopUpNewsletter = function (waitShow) {
                            if (_.checkCookie("euconsent")) {
                                var showPopup = false;

                                if (that.showCountdown <= 0) showPopup = true;

                                if (showPopup == false) {
                                    if (_.checkCookie("countdown")) {
    ...
    var Mangiro = function () {
        ...
        function Mangiro(o) {
            ...
            this.scrolltoanchor = new ScrollToAnchor(o.ScrollToAnchor);
            this.scrolltoindex = new ScrollToIndex(o.ScrollToIndex);
    ...
    var ScrollToAnchor = function () {
        ...
        function ScrollToAnchor(o) {
            this.offsetDesktop = 90;
            this.offsetMobile = 30;
            this.duration = 300;
            o = o || {};
            o = o.ScrollDetector || o;
            $.extend(this, o);
        ...
        $.extend(ScrollToAnchor.prototype, {
            init: function () {
                var that = this;

                $('.content_index .anchor').click(function (event) {
                    event.preventDefault();
                    var href = $.attr(this, 'href');
                    var offset = !isMobile.any() ? that.offsetDesktop : that.offsetMobile;
                    var scrollEnd = $(href).offset().top - offset;
                    $('html, body').animate({
                        scrollTop: scrollEnd
                    }, that.duration);
                });
    ...
    var ScrollToIndex = function () {
        "use strict";
        function ScrollToIndex(o) {
            this.offsetDesktop = 90;
            this.offsetMobile = 30;
            this.duration = 300;
            this.index = ".content_index";
            o = o || {};
            o = o.ScrollDetector || o;
            $.extend(this, o);
        }

        $.extend(ScrollToIndex.prototype, {
            init: function () {
                var that = this;
                $('.goto-index').each(function (event) {
                    $(this).on("click", function (event) {
                        event.preventDefault();
                        var offset = !isMobile.any() ? that.offsetDesktop : that.offsetMobile;
                        var scrollEnd = $(that.index).offset().top - offset;
                        $('html, body').animate({
                            scrollTop: scrollEnd
                        }, that.duration);
                    });
    ...
    /*
    "use strict";
    console.log("Inicia el ScrollToIndex");
    


    $('.goto-index').click(function (event) {
        event.preventDefault();
        console.log("oye");
        var href = $(this).closest('*[id^="cke"]');
        console.log(href);
        var offset = isMobile.any() ? that.offsetMobile : that.offsetDesktop;
        var scrollEnd = $(href).offset().top - offset;
        $('html, body').animate({
            scrollTop: scrollEnd
        }, that.duration);
